...
local function ParseMessage(chatmsg)
    ...
    if not faction then
        ...
        if amount then amount = -amount end
    end
    if not faction then
        return nil
    end
	for i = 1, GetNumFactions() do
		...
		if name == faction then return factionId, amount end
	end
	return nil
end
...
local function GetStanding(factionId)
	...
	if     value <  -6000 then value = value + 42000
	elseif value <  -3000 then value = value +  6000
	elseif value <      0 then value = value +  3000
	elseif value <   3000 then value = value +     0
	elseif value <   9000 then value = value -  3000
	elseif value <  21000 then value = value -  9000
	elseif value <  42000 then value = value - 21000
	elseif value <  42999 then value = value - 42000
	end
...
function MoldyReputation:ShowReputationDetails(...)
	...
	for i = 1, GetNumFactions() do
		local _, _, _, _, _, _, _, _, _, _, _, _, _, id = GetFactionInfo(i)
		if factionId == id then
			ToggleCharacter("ReputationFrame", true)
			ReputationBar_OnClick(_G["ReputationBar"..i])
			return
		end
	end
end
...
function MoldyReputation:CHAT_MSG_COMBAT_FACTION_CHANGE(event, chatmsg, ...)
	...
	if not self.stats.session[factionId] then self.stats.session[factionId] = { total = 0 } end
	self.stats.session[factionId].total = self.stats.session[factionId].total + change
end

function MoldyReputation:OnEnable()
	self:RegisterEvent("CHAT_MSG_COMBAT_FACTION_CHANGE")
	ChatFrame_AddMessageEventFilter("CHAT_MSG_COMBAT_FACTION_CHANGE", function(...) return self:ReputationChangeFilter(...) end)
	for i = 1, NUM_CHAT_WINDOWS do
		local frame = _G["ChatFrame"..i]
		if frame then
			frame:SetScript("OnHyperlinkEnter", function(...) self:ShowTooltip(...) end)
			frame:SetScript("OnHyperlinkLeave", function(...) self:HideTooltip(...) end)
			frame:SetScript("OnHyperlinkClick", function(...) self:ShowReputationDetails(...) end)
		end
	end
end