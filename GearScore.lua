...
local function GetItemScore(itemLink)
    ...
    if itemQuality == Enum.ItemQuality.Heirloom then itemLevel = 187 end
...
if score > MAX_SCORE then return MAX_SCORE end
return score
end

local function GetColor(score)
    if score > MAX_SCORE then score = MAX_SCORE end
    ...
    for slot = INVSLOT_FIRST_EQUIPPED, INVSLOT_LAST_EQUIPPED do
        ...
        if itemLink then
            ...
            if UnitClassBase(unit) == "HUNTER" then
                if slot == INVSLOT_MAINHAND then temp = temp * 0.3164 end
                if slot == INVSLOT_OFFHAND  then temp = temp * 0.3164 end
                if slot == INVSLOT_RANGED   then temp = temp * 5.3224 end
            end
            if slot == INVSLOT_MAINHAND then temp = temp * titanGrip end
            if slot == INVSLOT_OFFHAND  then temp = temp * titanGrip end
            score = score + temp
        end
    end
    score = floor(score)
